...
function selectFlag(language) {
  const imgSrc = `img/${language}.png`;
  selected.innerHTML = `<img src="${imgSrc}" alt="${language}" />`;
...
const textLoad = () => {
    setTimeout(() => {
        text.textContent = "Servus, ich bin";
    }, 0);
    setTimeout(() => {
        text.textContent = "Hi, i'm ";
    }, 4000);
    setTimeout(() => {
        text.textContent = "Bonjour, je suis";
    }, 8000); 
}
textLoad();
